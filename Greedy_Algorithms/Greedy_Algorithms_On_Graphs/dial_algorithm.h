#pragma once
#ifndef _DIAL_ALGORITHM_H
#define _DIAL_ALGORITHM_H
...
void Graph::shortestPath(int src, int w) {
	...
	int idx = 0;
	while (1) {
		// Go sequentially through buckets until one non-empty
		// bucket is found
		...
		// update their distance if required
		for (auto i = adj[u].begin(); i != adj[u].end(); i++) {
			...
			// If there is shorted path to b through u.
			if (dv > du + weight) {
